...
def get_doc_to_norm(index, doc_freq, num_docs):
    ...
    for term in index.keys():
        for doc_id, doc_tf in index[term]:
            # use the formula of TF-IDF Variant in Q2
            doc_idf = np.log(num_docs / (1 + doc_freq[term]))
            doc_norm[doc_id] += (doc_tf * doc_idf) ** 2
...
def run_query(query_token_counts, index, doc_freq, doc_norm, num_docs):
    ...
    for query_term, query_tf in query_token_counts:
        ...
        # guard the exceptional case that when the frequency is 0
        if query_term not in doc_freq.keys():
            doc_freq[query_term] = 0

        # tf-idf variant formula
        query_idf = np.log(num_docs / (1 + doc_freq[query_term]))
        query_norm += (query_tf * query_idf) ** 2
    ...
    for query_term, query_tf in query_token_counts:
        ...
        # tf-idf variant formula
        doc_idf = np.log(num_docs / (1 + doc_freq[query_term]))
        for doc_id, doc_tf in index[query_term]:
            doc_to_score[doc_id] += (query_tf * doc_idf) * (doc_tf * doc_idf) / (doc_norm[doc_id] * query_norm)
